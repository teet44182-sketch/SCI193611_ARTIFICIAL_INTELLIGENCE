...
def blind_dog_program(percepts):
    for p in percepts:
...
class Park(Environment):
    ...
    def execute_action(self, agent, action):
        if action == "move down":
            print(f'{agent.__class__.__name__} decided to {action} at location: {agent.location}')
            agent.movedown()
        elif action == "eat":
            items = self.list_things_at(agent.location, tclass=Food)
            if len(items) != 0:
                if agent.eat(items[0]):
                    print(f'{agent.__class__.__name__} ate {items[0].__class__.__name__} at location: {agent.location}')
                    self.delete_thing(items[0])
        elif action == "drink":
            items = self.list_things_at(agent.location, tclass=Water)
            if len(items) != 0:
                if agent.drink(items[0]):
                    print(f'{agent.__class__.__name__} drank {items[0].__class__.__name__} at location: {agent.location}')
                    self.delete_thing(items[0])
...
if __name__ == "__main__":
    ...
    agent = ReflexVacuumAgent()
    env.add_thing(agent)
    print(f"  Start: {env.status}")
    env.run(steps=20)
    print(f"  End:   {env.status}")
    print(f"  Score: {agent.performance}")
    
    print("\n[2] ModelBasedVacuumAgent")
    env = TrivialVacuumEnvironment()
    agent = ModelBasedVacuumAgent()
    env.add_thing(agent)
    print(f"  Start: {env.status}")
    ...
    print("\n[5] BlindDog in Park")
    park = Park()
    dog = BlindDog(blind_dog_program)
    dogfood = Food()
    water = Water()
    ...
    print(f"  Dog starts at: {dog.location}")
    print(f"  Food at: {dogfood.location}")
    print(f"  Water at: {water.location}")
    print("  Running simulation...")
    
    park.run(10)
    
    print(f"  Dog final position: {dog.location}")
